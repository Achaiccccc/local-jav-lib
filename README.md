...
- ❤️ **收藏夹**: 图文列表页、影片详情页可点击收藏，支持多收藏夹；内置「默认收藏夹」和「最近看过」（30天内播放过的数据）列表
- 🧩 **分类管理**: 分类 Tab 支持编辑当前分类列表与一键导入默认分类，编辑弹窗中每个小类右侧展示「可播放/总数」统计，便于快速了解当前配置覆盖情况
- 🎰 **随机摸奖**: 选择困难症犯了？试试随机抽奖吧！支持在抽奖弹窗按分类与年份选择随机范围
...
- **视图模式**: 列表支持文字、图文、缩图三种主模式；图文与缩图均提供二级图片类型切换（封面图/海报图），默认封面图。
